    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            for (ex, ey, ew, eh) in eyes:
                ...
                # add text display open eyes ratio for eyes closed right frame text
                detected_eyes = [ear for status, ear in data_list if status == "Alert Eyes"]
                open_eyes_ratio = sum(1 for status, ear in data_list if status == "Alert Eyes") / len(detected_eyes) if len(detected_eyes) > 0 else 0
